...
| `<env>/global-secrets.yaml`   | **generated** by tofu (`output-file`) | **no** (gitignored) | all secrets (PG/Redis/auth passwords) |
...
> `global-secrets.yaml` never get committed (see `.gitignore`). State lives in S3.
...
- **Infra secrets** (Postgres admin/aggregator/dpg + Redis + auth passwords) are
  ...
  gitignored `global-secrets.yaml`, then stored at runtime in Kubernetes
