...
tfstate lives in an S3 bucket (encrypted, versioned, private) — never committed; `.terraform/`, `*.tfstate`, `*.tfvars`, generated `tf.sh`/`global-cloud-values.yaml`/`global-credentials.yaml` are all gitignored. App secrets are generated by `random_passwords` + `output-file` into the gitignored `global-credentials.yaml`; SMTP/MSG91/maps keys are plain anchors in `global-values.yaml` (non-secret-tier by current design).
