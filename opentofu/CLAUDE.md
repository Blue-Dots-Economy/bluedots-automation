...
Guidance for the OpenTofu/Terragrunt half of the repo. Read the root `CLAUDE.md` first (the install.sh dispatcher, the values-file architecture, the naming table). This file covers the infra layer specifically. `docs/deployment_guide.md` is the authoritative end-to-end runbook + `install.sh` command reference.
...
Three secrets in `global-secrets.yaml` are **raw service api keys** (`x-api-key`, verified against signals' `apikey` table — originally better-auth's, kept as a cross-repo contract when signals-dpg#757 removed the library) rather than passwords or OIDC client secrets: `signalstack_admin_key` (rendered as `AGGREGATOR_DPG_API_KEY`), `signals_search_api_key`, and `raya_voice_bot_api_key`. They share a shape — `random_password`, `special = false`, length from a `*_api_key_length` knob defaulting to **48** with a **`>= 32` validation** — because the signals api migrate-job's `provision_service_users.sql` rejects anything shorter. Only the `base64url(sha256())` hash is ever stored in the database; see `helm/CLAUDE.md` → *Service apikeys* for the consuming side.
...
- **`pritunl`** — Pritunl OpenVPN server (Ubuntu 22.04 + MongoDB via cloud-init) in a **public** subnet with an Elastic IP; the single front door routing the VPC CIDR to connected laptops. Its SG splits the two surfaces, because they are different risks. **OpenVPN 1194 UDP+TCP** takes `pritunl_vpn_ingress_cidrs`, default `0.0.0.0/0` and **deliberately left open**: Pritunl authenticates every connection with a per-user client certificate, so the cert is the access control and a source-IP gate there buys nothing while locking out every home/mobile user the moment their ISP lease moves (this repo carried three different "office" IPs across three env dirs — that drift is the failure mode). **SSH 22 + web-admin 443** take `pritunl_admin_ingress_cidrs`, and the module always appends `var.vpc_cidr` on top, so the admin surface stays off the internet but is reachable on the host's private IP to anyone already on the VPN — a public CIDR there is only for the one-time bootstrap, and a stale one is recoverable by widening the SG in the AWS console, which is not VPN-gated. `_common/pritunl.hcl` falls back to the legacy single-list `pritunl_ingress_cidrs` key for `admin_ingress_cidrs`, so a per-deployment branch written before the split keeps its office CIDRs on the admin ports (and only there) with no edit. `t3.small` minimum (MongoDB RAM). It has `lifecycle.ignore_changes` on `associate_public_ip_address` so day-2 SG edits don't recreate it (recreation would wipe the hand-configured Pritunl org/user/server — set up once by hand over SSH).
...
Access is **SSH public key only** (`bastion_authorized_keys`, shared by both hosts; private keys stay with devs, so nothing secret lands in tfstate). Add/remove a key or CIDR in `global-values.yaml` and re-apply `bastion`/`pritunl`. Note the VPN itself is **not** IP-gated — see the `pritunl` bullet above. To go fully private-endpoint: set both `eks_endpoint_public_access` and `eks_endpoint_private_access` `true`, verify from the bastion, then flip public access `false`.
