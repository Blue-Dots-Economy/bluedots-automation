...
**The signals half is currently inert.** It guards `POST /api/auth/unified-otp/request`, better-auth's OTP-send endpoint, and signals-dpg#757 removed the whole `/api/auth/*` mount, so that route now 404s and `_signals_otp_per_minute` limits nothing. Login OTP is sent by the Keycloak OTP SPI, so the **keycloak** limiter (`_aggregator_otp_per_minute`) is the one that protects every login, signals' included. Signals still sends **guardian** OTPs itself (`POST /api/v1/consent/u18/guardian`, `/api/v1/consent/u18/signup/guardian`); at Kong those get only the 120/min `/api/v1/consent` group plus the app's per-contact caps. The Ingress is kept, deliberately, until someone decides whether to repoint it at those paths or delete it. Do not assume it protects anything today.
...
**The groups list is not self-maintaining and fails open.** It must match the api's mounted prefixes (`apps/api/src/routes/v1/v1_routes.ts`). A prefix that is missing does not error — it falls through to the per-service catch-all tier (`rl-signals-api`, 10000/min), i.e. effectively unlimited. The app-level `/api/auth` mount (better-auth's own HTTP surface) was removed in signals-dpg#757 and has been dropped from the list; `/api/v1/auth` is a **different** prefix and stays. Kong matches on path prefix, so the two never covered each other.
