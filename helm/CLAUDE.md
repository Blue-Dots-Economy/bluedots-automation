...
**The signals half goes inert with signals-dpg#757 — but only once that release is deployed.** It guards `POST /api/auth/unified-otp/request`, better-auth's OTP-send endpoint. #757 removes the whole `/api/auth/*` mount, but deployed environments pin images that predate it, so this limiter is **still live** today, and on an environment still on `AUTH_PROVIDER=betterauth` (OneTAC) it guards the actual login OTP path. The same applies to the `/api/auth` entry in `apiRateLimit.groups`: keep both until a #757 release is on every environment, then remove them with OneTAC's cutover (bluedots-infra-deployments#32). Dropping the group early silently moves `/api/auth/*` to the 10000/min catch-all. After #757, login OTP is sent by the Keycloak OTP SPI and limited by the **keycloak** limiter (`_aggregator_otp_per_minute`). Signals still sends **guardian** OTPs itself (`POST /api/v1/consent/u18/guardian`, `/api/v1/consent/u18/signup/guardian`), which at Kong get only the 120/min `/api/v1/consent` group plus the app's per-contact caps; at that point, decide whether to repoint this limiter at those paths or delete it.

**SSL-redirect + HSTS (#128, Issue #10).** Every browser- and API-facing Ingress in `helm/signals` (`ui`, `api`, `search`), `helm/aggregator` (`web`, `api`) and `helm/keycloak` carries `ingress.forceSSLRedirect` + `ingress.hsts.*`, both default **on**, no-op unless `ingressClassName` is `kong`. Redirect adds `konghq.com/protocols: "https"` + `konghq.com/https-redirect-status-code`; HSTS is a per-chart `response-transformer` `KongPlugin` (`{fullname}-hsts`) injecting `Strict-Transport-Security`. The HSTS plugin is **combined** into the ingress's `konghq.com/plugins` list alongside the existing rate-limit-tier reference (comma-joined, not replacing it).

**Every route in a chart needs it, not just the catch-all.** Kong serves the most specific matching route and applies only *that* route's plugin set, so the generated more-specific Ingresses must re-attach the HSTS plugin and the protocol annotations themselves: `api-endpoint-groups-ingress.yaml` (per-API rate-limit groups, in signals-api/search/aggregator-api), signals-api's `otp-ratelimit-ingress.yaml`, and keycloak's `otp-ratelimit-ingress.yaml` + `admin-console-block-ingress.yaml`. Miss one and the highest-traffic and most sensitive API paths are precisely the ones answering over plain http with no header. **Add a new more-specific Ingress to any of these charts and it needs the same two blocks.** (Keycloak's admin-console block answers via `request-termination` in the access phase, so the response header may not reach the 403 itself; the force-SSL annotations are route-level and apply regardless, which is the part that matters there.)

`httpsRedirectStatusCode` is **308 on the api-ish charts (signals api + search, aggregator api, keycloak), 301 on the web/ui charts**. API routes carry POSTs (e.g. the aggregator-registration decision endpoint) and clients have historically re-issued a 301'd POST as a GET, silently turning a write into a no-op; 308 requires method and body to be preserved. Kong accepts only 426/301/302/307/308 and rejects the route outright otherwise, so each chart's main ingress template `fail`s the render on any other value rather than letting the route disappear at admission.

`hsts.preload` defaults to **false** and is opt-in per environment. It is the one directive that cannot be walked back on browser timescales: with `includeSubDomains`, a preload-list submission commits every subdomain to https-only and removal ships with browser releases months later. The header alone does not enrol a domain (that needs a submission at hstspreload.org), but do not turn it on without auditing every subdomain of the public host first.

ACME http-01 is unaffected — cert-manager's own solver Ingress serves the challenge path over http, separate from these routes' https-only restriction (verify on first renewal). Keycloak normally shares the aggregator's host, whose `web` Ingress already sets HSTS for that name — HSTS is host-scoped, so its own block matters for the dedicated-host case (`global.keycloak.host`) and for a client whose first request is to `/auth`.

**Per-API rate-limit groups: `limit` is a cap, not an absolute.** `apiRateLimit.groups` entries render as extra Kong routes with their own per-IP counters, and an entry's `limit` is clamped to `min(limit, defaultMinute)` in `api-endpoint-groups-ingress.yaml`. An override exists to make one path *tighter* than the baseline, and a hardcoded number cannot do that portably: the dev cluster runs `_api_rate_limit_per_minute: 60` while the committed template default is 600, so an unclamped `limit: 120` on the pre-login paths would make them the **loosest** routes on the service there — the exact inverse of why the override was written. To genuinely loosen one path, raise the default and cap the others.

**The groups list is not self-maintaining and fails open.** It must match the api's mounted prefixes (`apps/api/src/routes/v1/v1_routes.ts`, plus the app-level `/api/auth` in `app.ts`). A prefix that is missing does not error — it falls through to the per-service catch-all tier (`rl-signals-api`, 10000/min), i.e. effectively unlimited. Note `/api/auth` (better-auth's own mount) and `/api/v1/auth` are **different prefixes**; Kong matches on path prefix, so listing one does not cover the other. signals-dpg#757 removes `/api/auth`, but its entry stays until a release containing #757 is deployed everywhere (see the OTP note above).
...
`provision_service_users.sql` seeds one org + user + `apikey` row per integrating service, hashing the raw key as `base64url(sha256(raw))`. That hash is a **cross-repo contract**, not a better-auth artefact: signals-dpg#757 removed better-auth but kept the `apikey` table and this exact format (byte-identical to the retired `@better-auth/api-key` `defaultKeyHasher`), because the api's in-process verifier (`plugins/auth/verify_api_key.ts`) and signals-search both check keys against it. Changing the format rotates every partner key at once. **Only the hash reaches the database** — the raw key lives solely in the generated `global-secrets.yaml` and in the caller's own config. Rotation is `UPDATE … WHERE user_id`, keyed per service user, so rotating one service never disturbs another.
...
**`RAYA_API_KEY` is not signals' `RAYA_VOICE_BOT_API_KEY`.** That one is *inbound* (the raya bot → the signals api, an `x-api-key` seeded into signals' `apikey` table, generated by `random_passwords`), and neither is `secrets.voiceDpgSignalsSecret`, the same bot's Keycloak client-credentials secret. Three separate values for one vendor, each independently rotatable — see *Service apikeys* above.
