...
ACME http-01 is unaffected — cert-manager's own solver Ingress serves the challenge path over http, separate from these routes' https-only restriction (verify on first renewal). `helm/signals/charts/search` and `helm/keycloak` do **not** carry HSTS yet.
